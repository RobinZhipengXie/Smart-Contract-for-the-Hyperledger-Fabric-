...
class PatientRecordContract extends Contract {
  ...
  async afterTransaction(ctx) {
    console.log(
      '---------------------INSIDE afterTransaction-----------------------'
    )
    let func_and_params = ctx.stub.getFunctionAndParameters()
    console.log('---------------------func_and_params-----------------------')
    console.log(func_and_params)
    console.log(
      func_and_params['fcn'] === 'createPatientRecord' &&
        func_and_params['params'][4] === 'AB-'
    )
    if (
      func_and_params['fcn'] === 'createPatientRecord' &&
      func_and_params['params'][4] === 'AB-'
      ...
      ctx.stub.setEvent(
        'rare-blood-type',
        JSON.stringify({ username: func_and_params.params[0] })
      )
  ...
  async createPatientRecord(ctx, username, name, dob, gender, blood_type) {
    let precord = PatientRecord.createInstance(
      username,
      name,
      dob,
      gender,
  ...
  async queryByGender(ctx, gender) {
    ...
    // peer chaincode query -C -n -c '{"Args":["queryByGender", "{\"selector\":{\"gender\":\"gender\"}, \"use_index\":[\"_design/genderIndexDoc\", \"genderIndex\"]}"]}'

    // let queryString = { selector: { gender: gender } }
    let queryString = {
      selector: {
        gender: gender,
      },
      use_index: ['genderIndexDoc'],
    }
    let queryRes = await this.queryWithQueryString(
      ctx,
  ...
  async queryByBlood_Type(ctx, blood_type) {
    ...
    //      Pass the Query string built to queryWithQueryString
    // let queryString = { selector: { blood_type: blood_type } }
    let queryString = {
      selector: {
        blood_type: blood_type,
      },
      use_index: ['blood_typeIndexDoc'],
    }
    let queryRes = await this.queryWithQueryString(
      ctx,
      JSON.stringify(queryString)
  ...
  async queryByBlood_Type_Dual(ctx, blood_type1, blood_type2) {
    ...
    // let queryString = {
    //   selector: { blood_type: blood_type1, blood_type: blood_type2 },
    //   use_index: ['_design/blood_type_dualIndexDoc', 'blood_type_dualIndex'],
    // }
    let queryString = {
      selector: {
